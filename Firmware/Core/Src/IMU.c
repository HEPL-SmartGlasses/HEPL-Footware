...
void IMU_updateOffsetCorrections(void) {
	...
	IMU_readRegister(OUTX_L_XL, buffer, 6);
...
HAL_StatusTypeDef IMU_readRegister(uint8_t reg_addr, uint8_t* rx_buf, int num_bytes) {
	...
	HAL_SPI_Transmit(IMU_config.hspi, (uint8_t *)reg_buffer, 1, SPI_TIMEOUT);
	HAL_SPI_Receive(IMU_config.hspi, (uint8_t *)rx, num_bytes, SPI_TIMEOUT);
...
HAL_StatusTypeDef IMU_writeRegister(uint8_t* tx_buf, int num_bytes) {
	...
	HAL_SPI_Transmit(IMU_config.hspi, (uint8_t *)tx_buf, num_bytes + 1, SPI_TIMEOUT);
